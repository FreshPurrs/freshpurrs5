...
function FAQItem({ question, answer }: FAQItemProps) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md border border-white/50 overflow-hidden">
      <button 
        className="w-full text-left p-6 flex justify-between items-center hover:bg-brand-primary-light/20"
        onClick={() => setIsOpen(!isOpen)}
      >
        <h3 className="font-bold text-lg text-brand-secondary-dark">{question}</h3>
        <i className={`fas ${isOpen ? 'fa-minus' : 'fa-plus'} text-brand-secondary ml-4`}></i>
      </button>
      {isOpen && (
        <div className="p-6 pt-0 border-t border-brand-secondary-light">
          {answer}
        </div>
      )}
    </div>
...
export default function FAQ() {
  return (
    <section id="faq" className="mb-16 md:mb-24">
      <h2 className="text-3xl font-bold text-center mb-4 text-brand-secondary">Frequently <span className="text-brand-primary">Asked Questions</span></h2>
            ...
            <>
              ...
              <ul className="space-y-2 text-gray-700">
                <li className="flex items-start">
                  <i className="fas fa-circle-notch text-xs text-brand-secondary mt-1 mr-2"></i>
          ...
          answer={
            <>
              ...
              <ul className="space-y-2 text-gray-700 mt-2">
                <li className="flex items-start">
                  <i className="fas fa-check-circle text-brand-green mt-0.5 mr-2"></i>
        ...
        <FAQItem 
          ...
          answer={
            <>
              ...
              <ul className="space-y-2 text-gray-700 mt-2">
                <li className="flex items-start">
                  <i className="fas fa-paw text-brand-secondary mt-0.5 mr-2"></i>
        ...
        <FAQItem 
          ...
          answer={
            <ul className="space-y-3 text-gray-700">
              <li className="flex items-start">
                <div className="bg-brand-secondary-light text-brand-secondary-dark rounded-full w-6 h-6 flex items-center justify-center mr-3 mt-0.5 flex-shrink-0 shadow-sm">
